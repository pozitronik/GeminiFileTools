	...
	try
		...
		begin
			...
			for I := 0 to High(LFile.RunSettings.SafetySettings) do
				WriteLn('  ', LFile.RunSettings.SafetySettings[I].Category, ' = ', LFile.RunSettings.SafetySettings[I].Threshold);
		end;
		...
		begin
			...
			if Length(LSysInstr) > 500 then
				LSysInstr := Copy(LSysInstr, 1, 500) + '... [truncated, ' + FormatByteSize(Length(LFile.SystemInstruction) * SizeOf(Char)) + ' total]';
			WriteLn(LSysInstr);
		end
		else
			WriteLn('(none)');
...
begin
	...
	try
		...
		WriteLn;
		WriteLn(Format('%-6s  %-8s  %-16s  %-12s  %-12s  %s', ['Index', 'Chunk#', 'MIME Type', 'B64 Size', 'Est. Size', 'Extension']));
		WriteLn(StringOfChar('-', 76));

		LTotalSize := 0;
		for I := 0 to High(LResources) do
		begin
			WriteLn(Format('%-6d  %-8d  %-16s  %-12s  %-12s  %s', [I, LResources[I].ChunkIndex, LResources[I].MimeType, FormatByteSize(LResources[I].Base64Size), FormatByteSize(LResources[I].DecodedSize), LResources[I].GetFileExtension]));
...
begin
	...
	begin
		...
		end else if SameText(LParam, '--sequential') then
			LThreaded := False
	...
	if LOutputDir = '' then
		LOutputDir := TPath.Combine(TPath.GetDirectoryName(TPath.GetFullPath(AFileName)), TPath.GetFileNameWithoutExtension(AFileName) + '_resources');

	if not TryLoadFile(AFileName, LFile) then
		Exit;
	try
		LFile.OnExtractProgress := procedure(AIndex, ATotal: Integer; const AFN: string)
			begin
