...
type
  ...
  IGeminiResourceExtractor = interface
    ...
    function ExtractAll(const AResources: TArray<TGeminiResource>;
      const AOutputDir: string; AThreaded: Boolean; const ANamePrefix: string;
      AOnProgress: TGeminiExtractProgressEvent): Integer;
  end;
...
function TGeminiResourceExtractor.ExtractAll(const AResources: TArray<TGeminiResource>;
  const AOutputDir: string; AThreaded: Boolean; const ANamePrefix: string;
  AOnProgress: TGeminiExtractProgressEvent): Integer;
var
  LAbsDir: string;
    ...
    TParallel.&For(0, Result - 1,
      ...
      begin
        LFN := TPath.Combine(LAbsDir,
          Format('%s_%.*d%s', [ANamePrefix, LPadWidth, AIdx, AResources[AIdx].GetFileExtension]));
  ...
  begin
    ...
    begin
      LFileName := TPath.Combine(LAbsDir,
        Format('%s_%.*d%s', [ANamePrefix, LPadWidth, I, AResources[I].GetFileExtension]));
